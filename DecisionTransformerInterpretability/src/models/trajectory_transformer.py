...
class TrajectoryTransformer(nn.Module):
    ...
    def initialize_state_embedding(self):
        ...
        envc = self.environment_config

        # ----- (A) existing CNN & ViT options --------------------------
        if cfg.state_embedding_type.lower() == "cnn":
            state_embedding = MiniGridConvEmbedder(cfg.d_model, endpool=True)

        elif cfg.state_embedding_type.lower() == "vit":
            state_embedding = MiniGridViTEmbedder(cfg.d_model)

        # ----- (B) NEW: flat linear layer for already‑flattened obs ---
        elif cfg.state_embedding_type.lower() == "flat":
            in_dim = int(np.prod(envc.observation_space.shape))
            state_embedding = nn.Linear(in_dim, cfg.d_model, bias=False)
            nn.init.normal_(state_embedding.weight, mean=0.0, std=0.02)

        # ----- (C) fallback: auto‑flatten whatever comes in ------------
        else:
            if isinstance(envc.observation_space, Dict):
                in_dim = int(np.prod(envc.observation_space["image"].shape))
            else:
                in_dim = int(np.prod(envc.observation_space.shape))
